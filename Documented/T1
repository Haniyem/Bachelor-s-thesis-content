 ...
 */
/**
 * @file DocumentsWriterFlushControl.java
 * @module org.apache.lucene.index
 *
 * @description
 * This file contains the implementation of DocumentsWriterFlushControl, which manages memory
 * accounting and coordinates flushing for per-thread document writers (DocumentsWriterPerThread)
 * in Lucene's indexing subsystem. It enforces configured RAM buffer limits and per-thread hard
 * limits, cooperates with a FlushPolicy to decide when writers should flush, and orchestrates
 * full flush operations used during commits and near-real-time refreshes.
 *
 * The class is responsible for:
 * - Tracking active and flush-pending memory usage.
 * - Marking thread-states as flush-pending and checking them out for flushing.
 * - Stalling and resuming indexing threads when memory pressure is high.
 * - Coordinating full flush semantics, including marking and collection of writers that must be flushed.
 *
 * @dependencies
 * - DocumentsWriterPerThread: per-thread indexing state and RAM accounting.
 * - DocumentsWriterPerThreadPool: pool of per-thread states used for checkout and registration.
 * - FlushPolicy: policy to determine when per-thread states should be flushed.
 * - DocumentsWriterStallControl: helper that stalls/resumes indexing threads under pressure.
 * - DocumentsWriter: owner object providing deleteQueue and other coordination.
 * - LiveIndexWriterConfig: configuration source for RAM limits and flush policy.
 *
 * @author Apache Lucene contributors (original)
 * @version 1.0
 * @license Apache License 2.0
 * @lastmodified 2025-09-14: Fully documented with Javadoc-style comments and inline explanations.
 */
 ...
 * @classdesc
 * Controls flushing of DocumentsWriterPerThread instances based on configured RAM limits,
 * the provided FlushPolicy, and runtime memory pressure. It accounts for active vs. flushing
 * bytes, manages queues of pending/blocked flushes, and coordinates full flush operations
 * performed during commits or NRT refreshes.
 *
 * This class is thread-safe and uses internal synchronization to protect shared counters and
 * queues. It also interacts with DocumentsWriterPerThreadPool to check out and return thread
 * states for flush processing.
 *
 * @example
 * DocumentsWriterFlushControl dwfc = new DocumentsWriterFlushControl(documentsWriter, config);
 * DocumentsWriterPerThread dwpt = dwfc.obtainAndLock();
 * try {
 *   // index documents...
 *   dwfc.doAfterDocument(dwpt);
 * } finally {
 *   dwpt.unlock();
 * }
 */
...
/**
 * @function method
 * @description Documentation for method()
 *
 * @returns {See method signature} 
 */

  /**
 * @function DocumentsWriterFlushControl
 * @description Constructor. Initialize flush control with owning DocumentsWriter and configuration.
 * @param {DocumentsWriter} documentsWriter - owner that provides delete queue and perThreadPool
 * @param {LiveIndexWriterConfig} config - index writer configuration (RAM limits, flush policy)
 */
